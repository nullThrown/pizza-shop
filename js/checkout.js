...
function renderCustomToppings(toppings, container) {
  ...
  toppings.forEach((topping) => {
    switch (topping.side) {
      case 'left':
        leftStr += `${topping.name}, `;
        break;
      case 'full':
        fullStr += `${topping.name}, `;
        break;
      case 'right':
        rightStr += `${topping.name}, `;
        break;
    }
  });
  leftStr = leftStr.trim().slice(0, -1);
  fullStr = fullStr.trim().slice(0, -1);
  rightStr = rightStr.trim().slice(0, -1);
  toppingsLeftP.textContent = leftStr;
  toppingsFullP.textContent = fullStr;
  toppingsRightP.textContent = rightStr;
